...
def calculate_order_size(symbol, price):
    try:
        amount_precision = 3
        return round(config.trade_size_usd / price, amount_precision)
    except Exception as e:
        logging.error("Error calculating order size: %s", e)
        return None
...
def execute_market_order(symbol, side, amount):
    ...
    url = BYBIT_REST_API_URL + endpoint
    ...
    for _ in range(3):
        try:
            ...
            if order_response['retCode'] == 0:
                ...
                logging.error(f"Bybit order error: {order_response}")
                return None
